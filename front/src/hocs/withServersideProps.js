	...
	return async (context) => {
		...
		if (detailQueryId !== undefined && digitsOnlyChecking(detailQueryId)) {
			...
			if (intDetailQueryId > 0 && intDetailQueryId <= numsOfMuseums) {
				...
				//[3]쿼리가 존재하며 정수이지만 범위가 올바르지 않을 때
				//(음의 정수/오버된 쿼리 처리)
		...
		//Errors List
		//쿼리는 있는데 정수만 가지고 있는게 아닌 경우 (ex: 60abc)
		if (detailQueryId !== undefined && !digitsOnlyChecking(detailQueryId)) {
			return await getServerSideProps(context).then(async (res) => {
				const item = '404';
				const pagePath = `/404`;
		...
		//쿼리가 없을 때 -> 일반 페이지들. 이건 알아서 에러라우팅 함.
		if (detailQueryId === undefined) {
			console.log('상세페이지 외 페이지로 갈 때 detailQueryId', detailQueryId);
